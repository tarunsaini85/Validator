...
commonValidator = {
	...
	validate : function(params){
		...
		if(fNames.constructor===Array){for(var x=0;x<fNames.length;x++){commonValidator.validInit(fNames[x],sBts,errClass,okClass,sOkClass,mBoxId,mBoxCnt,mBoxHid,inlineErr,dEvts)}}
		else{commonValidator.validInit(fNames,sBts,errClass,okClass,sOkClass,mBoxId,mBoxCnt,mBoxHid,inlineErr,dEvts)}
	},
		
	validInit : function(fN,sB,eC,oC,soC,mBoxId,mBoxCnt,mBH,inErr,dEvts){
		var z,frmElm=$n('form[name='+fN+']').currObj(), sBtns, rStat, noVldSt=false;
		...
		if(sB){
			...
			for(z=0;z<sBtns.length;z++){
				if(sBtns[z].attr('type')!='submit'){
					...
					else{sBtns[z].addEvent('click',function(){commonValidator.isVld=commonValidator.checkSubmit(frmElm,sB,eC,oC,soC,mBoxId,mBoxCnt,mBH,inErr)})}
		}}}$n(frmElm).submit(function(){(commonValidator.noVld)?rStat=true:rStat=commonValidator.checkSubmit(frmElm,sB,eC,oC,soC,mBoxId,mBoxCnt,mBH,inErr);return rStat})
	},
	...
	checkSubmit : function(fElm,sB,eC,oC,soC,mBoxId,mBoxCnt,mBH,inErr){
			...
			for(s=0;s<sBtns.length;s++){
				if(sBtns[s].attr('rel')!==null && sBtns[s].attr('rel')!='' && sBtns[s].attr('rel')!='noValidate'){
	...
	validators : {
		...
		checkedRadChk:function(elm,frm){
			var x, f=false, fElm=$n(frm).childrens('input[type="radio"]');
	...
	getSbtBtns : function(frmElm,sB){
		...
		if(frmElm.childrens('input[type="submit"]').length>0){els.push(frmElm.childrens('input[type="submit"]'))}
		if(frmElm.childrens('button[type="submit"]').length>0){els.push(frmElm.childrens('button[type="submit"]'))};
