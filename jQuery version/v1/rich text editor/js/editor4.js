var editorWindow=function(){
	...
	eW.abClass=new Array('edt_bold', 'edt_Itl', 'edt_undr', 'edt_iol', 'edt_iul', 'edt_jl', 'edt_jc', 'edt_jr');
		...
		getTextarea.hide();
		getTextareaParent.append(getTextarea);
		getTextarea.attr('id',getTextarea.attr('id')+'_edTa');
		...
		{ 
		var clickA=$n('<div>').attr({'id':'contextMenu'}).css({'position':'absolute', 'top':0, 'left':0, 'background':'#fff', 'border':'1px solid #000', 'display':'none'}).html('<a>Paste</a>');
		$n('body').append(clickA)
		
		}
		if(document.onclick){
			if(window.addEventListener)
			document.addEventListener('click', function(){$n('#contextMenu').hide()}, false);
			else
			document.attachEvent('onclick', function(){$n('#contextMenu').hide()});
		}
		else
			document.onclick = function(){$n('#contextMenu').hide()}
...
//		modalwin(500, 200, 'adLayer', document.getElementsByTagName('body')[0]);// for old lightbox
//		modalwin(500, 'adLayer', document.getElementsByTagName('body')[0]);// for new lightbox
	...
	eW.contextMenuDown=function(event){
		...
		eW.objCont.focus();
		setTimeout(function(){eW.rg=eW.saveRange()}, 400);
