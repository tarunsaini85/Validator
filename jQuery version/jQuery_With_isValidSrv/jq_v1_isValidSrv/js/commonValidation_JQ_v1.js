...
commonValidator = {
	...
	validInit : function(fN,sB,eC,oC,soC,mBoxId,mBoxCnt,mBH,inErr,dEvts,delay){
		var z,frmElm=$('form[name='+fN+']').get(0), sBtns, rStat, noVldSt=false,sbtFlg;
		if(!this.supportPlaceholder()){
			this.setDefaultValues(frmElm);
		}
		this.checkEvents(frmElm,eC,oC,soC,inErr,dEvts);
		if(sB){
			sBtns=commonValidator.getSbtBtns($(frmElm),sB);
			for(z=0;z<sBtns.length;z++){
				if(sBtns[z].attr('type')!='submit'){
					if(sBtns[z].attr('rel')!='' && sBtns[z].attr('rel')=='noValidate'){sBtns[z].on('click',function(){commonValidator.noVld=true;$(frmElm).submit()})}
	...
	setDefaultValues : function(fElm){
		var x,fEls=commonValidator.getFrmElms($(fElm),true);
	...
	checkEvents : function(fElm,eC,oC,soC,inErr,dEvts){
		var x,y,evts=null, fEls=commonValidator.getFrmElms($(fElm),false);
		for(x=0;x<fEls.length;x++){
			...
			if(fEls[x].attr('rel').split('|')[1] || dEvts){
				...
				if(evts){
					for(y=0;y<evts.length;y++){
						...
						if(edtr){
							if(edtr){
								var edtrObj = (navigator.userAgent.match('MSIE'))?edtr.obj:edtr.objContDoc;
								$(edtrObj).on(evts[y],function(vlds,obj,frm,eCp,oCp,soCp){
	...
	checkSubmit : function(fElm,sB,eC,oC,soC,mBoxId,mBoxCnt,mBH,inErr){
			var s,x,vlds,cV,chkVld=false,sbtCallB,fEls=commonValidator.getFrmElms($(fElm),false),sBtns=commonValidator.getSbtBtns($(fElm),sB);
			(mBH)?$('.mgBox').hide():''; $(fElm).attr('chk',false); commonValidator.erArry={};
			...
			if(commonValidator.lastErr && $(fElm).attr('chk')=='true'){
				if(mBoxId){
				...
				(commonValidator.lastErr.obj)?$(commonValidator.lastErr.objCont).focus():commonValidator.lastErr.focus();
			}
			else{(mBoxId)?$('#'+mBoxId).hide():''}
		return !chkVld;
	},
	...
	isValid : function(id){
		var obj=$('#'+id),x,vlds,cV,chkVld=false;
						...
						case 'checked' :
							if(argChk){
								if(obj.attr('type')=='checkbox'){
									...
									(argSts)?'':argSts=this.validators.checkedChk(obj,fPar.get(0));
								}
								else if(obj.attr('type')=='radio'){
									var fElm,fPar=obj;
									while(fPar.get(0).nodeName.toLowerCase()!='form'){fPar=fPar.parent()}
									(argSts)?'':argSts=this.validators.checkedRadChk(obj,fPar.get(0));
								}
							}
							else{
	...
	heighlightErrOk : function(vCode,obj,fElm,hTyp,eC,oC,soC,inErr){
		if(obj.obj){var etr=true;obj=$(obj.obj)}else{etr=false}
		var x,errLbl=null, errM=commonValidator.errs[vCode] || vCode, errI=obj.attr('id')+'_err', errN=obj.attr('name')+'_err', frLbs=$(fElm).find('.erLbl'), eArrIndx=obj.attr('id') || obj.attr('name'),errF=null;
		...
		if(hTyp=='err'){
			...
			if(errLbl){
				...
				if(!etr){
					...
					errM=errM.replace('[MinL]',minL).replace('[MaxL]',maxL).replace('[MinV]',minV).replace('[MaxV]',maxV).replace('[currVal]',obj.val().toString().replace('<','&lt;').replace('>','&gt;'));
				}
	...
	validators : {
		...
		checkedRadChk:function(elm,frm){
			...
			for(x=0;x<fElm.length;x++){if(fElm.eq(x).attr('name')==elm.attr('name')){(fElm.eq(x).get(0).checked==true)?f=true:'';}}
		...
		selectedChk:function(elm){return (elm.get(0).selectedIndex!=0)?false:true},
	...
	getFrmElms : function(frmElm,fg){
		...
		for(x=0;x<fElms.length;x++){
			...
			if(fg){if((nodNam=='input' || nodNam=='select' || nodNam=='textarea') && $(fElms[x]).attr('type')!='submit' && $(fElms[x]).attr('placeholder')!=''){els.push($(fElms[x]))}}
			else{if((nodNam=='input' || nodNam=='select' || nodNam=='textarea') && $(fElms[x]).attr('type')!='submit' && $(fElms[x]).attr('rel') && $(fElms[x]).attr('rel')!=''){els.push($(fElms[x]))}}
		}
		return els;
	},
	...
	getSbtBtns : function(frmElm,sB){
		...
		if(frmElm.find("input[type=submit]").length>0){els.push(frmElm.find("input[type=submit]"))}
		if(frmElm.find("button[type=submit]").length>0){els.push(frmElm.find("button[type=submit]"))};
	...
	processAltRel : function(eId,stat){
		if(stat=='s'){$('#'+eId).attr('rel',$('#'+eId).attr('altrel'))}
		else if(stat=='h'){($('#'+eId).attr('altrel'))?'':$('#'+eId).attr('altrel',$('#'+eId).attr('rel'));$('#'+eId).removeAttr('rel')}
	},
	...
	fillVal : function(arr){if(arr.constructor===Object){for(var x in arr){$('#'+x).val(arr[x]).get(0).style.color = ''}}}
}